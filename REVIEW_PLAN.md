...
## Naprawione critical findings, Grupa A (2026-07-02)

Weryfikacja: `pnpm type-check` (0 nowych błędów — 3 przedistniejące, niepowiązane błędy w `vite.config.ts`/`use-toast.ts`/`usePickerLogic.ts`, potwierdzone jako obecne przed tą sesją), `pnpm lint-only` (0 błędów), `pnpm build` (buduje się czysto).

1. **2FA całkowicie zepsute end-to-end — 3 niezależne bugi.** `is2faRequested` nigdy nie był podpięty do żadnej trasy (był też napisany pod starą sygnaturę middleware) — przepisany i wpięty globalnie w `runMiddlewarePipeline` (działa dla każdej nawigacji, nie tylko tras z jawnie dodanym middlewarem — bez tego trzeba by edytować 33+ definicji tras). Backendowy `TwoFactorAuthController::verify()` zwraca `{accessToken, tokenType}`, ale typ `MfaVerifyResponse` deklarował `{message}`, a `2faVerifyPage.vue` nigdy nie zapisywał nowego tokenu przez `authStore.setToken()` — poprawiona weryfikacja kończyła się nieskończoną pętlą 403. Dodano też `interceptTwoFactorRequired` jako backstop dla `actionRequired: 'verify-2fa'` z backendu (ten sam wzorzec store-flag + watcher w layoucie co istniejący `interceptTenantRequired`/`SelectTenantModal`).
2. **`DeleteInvoiceAction.vue` — shadowing zmiennej czyścił całą listę faktur po usunięciu jednej.** `invoice.id !== invoice.id` (parametr callbacku przesłaniał prop) zawsze `false`. Naprawione przez zmianę nazwy parametru.
3. **Statyczny nagłówek `Accept-Language` + zahardkodowany fallback błędu.** Nagłówek ustawiany raz przy tworzeniu instancji axios, nigdy nie aktualizowany przy zmianie języka — wszystkie komunikaty backendu (w tym nowe komunikaty odmowy uprawnień z całego tego przeglądu) zawsze po angielsku niezależnie od UI. Przeniesiony do `authorizeOutgoingRequests.ts`, ustawiany świeżo przy każdym requeście z `useLanguageStore()`. `handleErrorWithToast` (115 miejsc użycia) miał zahardkodowany `'Unknown error'` — zastąpiony `i18n.global.t('common.unknownError')`. Usunięto też nonsensowny nagłówek żądania `Access-Control-Allow-Origin: *` (to nagłówek odpowiedzi, nie żądania).
4. **Callback OAuth ignorował błąd odrzucenia konta.** Backend przekierowuje z `?error=account_exists` gdy mail jest już zarejestrowany pod innym dostawcą — frontend czytał tylko `jwtToken`, user zawsze widział ogólny "Invalid OAuth2 callback". Dodano obsługę + i18n dla całej strony (wcześniej całkowicie po angielsku, bez `t()`).
5. **Cały dropdown akcji faktury (poza PDF/Delete) to były atrapy UI.** `ChangeStatusAction` wpięty do prawdziwego `PATCH /invoices/{id}` (lista statusów też była zła — `draft/sent/paid/overdue/cancelled` zamiast realnego enuma `draft/processing/issued/completed/cancelled` — każda zmiana zostałaby odrzucona przez backend nawet po wpięciu). Pozostałe 8 akcji (Copy, ShareLink, ManagePayment, ExportBankTransfer, ManageReminders, ManageRelationships, RecurringInvoice, SendToKsef, SendEmail) nie mają żadnego backendowego endpointu (potwierdzone względem `routes/api/invoices.php`) — zamiast dalej udawać sukces przez `setTimeout` + fałszywy toast, teraz pokazują uczciwy komunikat "funkcja niedostępna". Budowanie tych 8 od zera to osobny epik (niektóre wymagają decyzji produktowych, np. czym jest "kopia" faktury przy unikalnej numeracji; KSeF nie ma żadnej integracji backendowej w ogóle; ShareLink może już tworzyć prawdziwy token — Faza 3 backendu — ale nie ma publicznego endpointu do jego odczytu).

**Świadomie NIE naprawione w tej sesji (odłożone jako known-issue):** brak jakiegokolwiek client-side role-gatingu (systemowy brak w całej aplikacji, potwierdzony przez wszystkie 5 grup audytu — zbyt szeroki zakres na tę sesję), `AddProjectPage.vue` brak pola `statusId` (powiązane z backendowym known-issue całej domeny Projects), zamrożony token JWT w Echo/Pusher (WebSocket), demo-placeholdery w `CreateFeedForm`/`CommentForm`, martwy kod (`StripeMessages.vue`, `SubscriptionService.buy()`), pozostałe drobne i18n/dead-code z audytu.
...
- **[CRITICAL]** Blokada pól finansowych na fakturach COMPLETED/CANCELLED — **naprawione (backend + UI)**: `UpdateInvoiceRequest` używa camelCase (`totalNet` itd.); frontend `EditInvoicePage` blokuje lines/currency/number gdy `isInvoiceFinanciallyLocked(status)`.
