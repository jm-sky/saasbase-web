...
const attachments = ref<ITenantAttachment[]>([])
...
const refresh = async () => {
  try {
    loading.value = true
    const res = await tenantAttachmentsService.index(tenantId)
...
const handleUpload = async () => {
  ...
  try {
    ...
    for (const file of files.value) {
      await tenantAttachmentsService.upload(tenantId, file)
    }
    files.value = []
    await refresh()
  } catch (error) {
...
const handleDelete = async (attachment: ITenantAttachment) => {
  if (!confirm(t('attachments.delete.confirm'))) return
  try {
    await tenantAttachmentsService.delete(tenantId, attachment.id)
    await refresh()
  } catch (error) {
...
const handleDownload = async (attachment: ITenantAttachment) => {
  const blob = await tenantAttachmentsService.download(tenantId, attachment.id)
