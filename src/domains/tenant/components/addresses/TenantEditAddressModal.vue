...
const { tenantId, address } = defineProps<{
  tenantId: string
  address: ITenantAddress
}>()

const emit = defineEmits<{
  update: [ITenantAddress]
}>()

const { handleSubmit, values, setErrors, setValues, setFieldValue, isSubmitting } = useForm<ITenantAddress>({
...
const onSubmit = handleSubmit(async (values: ITenantAddress) => {
  try {
    const updatedAddress = await tenantAddressesService.update(tenantId, address.id, values)
    emit('update', updatedAddress)
    open.value = false
  } catch (error) {
    if (isValidationError(error)) setErrors(error.response.data.errors)
    handleErrorWithToast(t('tenant.addresses.edit.error'), error)
  }
})
...
<template>
  <ModalComponent
    :title="t('tenant.addresses.edit.title')"
    :description="t('tenant.addresses.edit.description')"
    :open="open"
    size="lg"
